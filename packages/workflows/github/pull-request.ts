...
export const processPullRequest = inngest.createFunction(
  ...
  async ({ event, step }: { event: any; step: any }) => {
    ...
    const pr = await step.run("process-pr", async () => {
      return await githubPullRequestService.processPullRequestEvent(installationId, repo.id, payload);
    });
    ...
    if (pr?.featureId) {
      await step.run("log-development-event", async () => {
        ...
        else if (payload.action === "closed" && payload.pull_request.merged) eventType = "PR Merged";
