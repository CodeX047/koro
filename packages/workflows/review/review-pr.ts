...
import {
  ...
  searchPrContext,
} from "@repo/services/review/vector";
  ...
  async ({ event, step }: { event: any; step: any }) => {
    ...
    const repository = await step.run("fetch-repository", async () => {
      const [repo] = await db
        ...
        .where(eq(repositoriesTable.id, pullRequest.repositoryId));
      return repo;
    });
    ...
    const alreadyReviewed = await step.run("check-already-reviewed", async () => {
      const [run] = await db
        ...
        .where(
          and(
            eq(reviewRunsTable.prId, pullRequestId),
            eq(reviewRunsTable.headSha, pullRequest.headSha),
          ),
        )
        .limit(1);
      return !!run;
    ...
    const repoContextSnippets = await step.run("search-repo-context", async () => {
      const [repoSync] = await db
        .select()
        .from(repoSyncTable)
        .where(eq(repoSyncTable.repoFullName, pullRequest.repoFullName));
      if (!repoSync || repoSync.status !== "synced") return [];
      return searchPrContext(
        `${pullRequest.repoFullName.replace("/", "--")}--codebase`,
        pullRequest.title,
      );
    });
    ...
    const featureContext = await step.run("fetch-feature-context", async () => {
      ...
      if (pullRequest.featureId) {
        const [prd] = await db
          .select()
          .from(prdsTable)
          .where(eq(prdsTable.featureId, pullRequest.featureId))
          .limit(1);
        ...
        const tasks = await db
          .select()
          .from(tasksTable)
          .where(eq(tasksTable.featureId, pullRequest.featureId));
        if (tasks.length > 0) {
