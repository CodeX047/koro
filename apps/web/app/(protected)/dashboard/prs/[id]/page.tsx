        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            {prData.reviewStatus && (
              <span
                className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                  prData.reviewStatus === "COMPLETED"
                    ? "bg-blue-500/10 text-blue-400"
                    : prData.reviewStatus === "RUNNING"
          ...
          <p className="text-slate-400 text-sm flex items-center gap-4">
            ...
            >
              <Github className="w-4 h-4" />#{prData.prNumber} in {prData.repoFullName}
            </a>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden">
              <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  {latestRun.verdict === "APPROVE" ? (
                    <CheckCircle2 className="w-5 h-5 text-green-400" />
                  ) : latestRun.verdict === "REQUEST_CHANGES" ? (
              ...
              <div className="p-4 space-y-4">
                <p className="text-slate-300">
                  {latestRun.scoreBreakdown?.summary || "Review completed."}
                </p>
                <div className="grid grid-cols-5 gap-2 text-center text-xs">
                  ...
                  <div className="bg-slate-900 p-2 rounded">
                    ...
                    <div className="font-bold">
                      {latestRun.scoreBreakdown?.maintainability || 0}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              ...
              <span className="ml-4 text-sm font-normal text-slate-400 flex items-center gap-2">
                <span className="text-green-400">
                  +{files.reduce((sum: number, f: any) => sum + (f.additions || 0), 0)}
                </span>
                <span className="text-red-400">
                  -{files.reduce((sum: number, f: any) => sum + (f.deletions || 0), 0)}
                </span>
              </span>
            </h2>
                  ...
                  <div key={run.id} className="flex gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500 mt-1">
                        <code className="bg-slate-900 px-1 py-0.5 rounded text-indigo-400/70">
