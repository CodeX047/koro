...
```typescript
const balance = await client.creditEntitlements.balances.get("cent_credit_id", "cus_abc123");
...
await client.creditEntitlements.balances.createLedgerEntry("cent_credit_id", "cus_abc123", {
  type: "credit",
  amount: "500",
...
const allBalances = await client.creditEntitlements.balances.list("cent_credit_id");

// Get full transaction history
const ledger = await client.creditEntitlements.balances.listLedger("cent_credit_id", "cus_abc123");
...
// List credit grants
const grants = await client.creditEntitlements.balances.listGrants("cent_credit_id", "cus_abc123");
```
...
export async function POST(req: NextRequest) {
  ...
  try {
    const entry = await client.creditEntitlements.balances.createLedgerEntry(creditId, customerId, {
      type: "credit",
...
async function grantPromoCredits(
  ...
  await client.creditEntitlements.balances.createLedgerEntry(creditId, customerId, {
    type: "credit",
    amount: amount.toString(),
