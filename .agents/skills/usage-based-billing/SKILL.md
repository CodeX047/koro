...
// Track single event
async function trackUsage(customerId: string, eventName: string, metadata: Record<string, string>) {
  await client.usageEvents.ingest({
    events: [
      {
...
async function trackImageGeneration(customerId: string, imageSize: string, model: string) {
  await client.usageEvents.ingest({
    events: [
      {
...
const balance = await client.creditEntitlements.balances.get("cent_ai_credits", "cus_abc123");
